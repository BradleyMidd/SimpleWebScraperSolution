    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                {
                    string content = client.DownloadString($"https://{craigslistCity.Replace(" ", string.Empty)}.craigslist.org/{Method}/{craiglistCategoryName}#search=1~gallery~0~0");

                    ScrapeCriteria scrapeCriteria = new ScrapeCriteriaBuilder()
                        .WithData(content)
                        .WithRegex(@"<a class=\""titlestring\"" href=\""(.*?)\"">(.*?)</a>")
